...
<div class="postbox">
  ...
  <div class="inside"><?php $wp2pcs_load_linktype = (int)get_option('wp2pcs_load_linktype');$wp2pcs_outlink_code = get_option('wp2pcs_outlink_code');$wp2pcs_site_id = get_option('wp2pcs_site_id');global $wp_rewrite;  ?>
    ...
    <p><label><input type="radio" name="wp2pcs_load_linktype" value="2" <?php checked($wp2pcs_load_linktype,2); ?> <?php if(!$wp2pcs_outlink_code || !$wp2pcs_site_id) echo 'disabled'; ?>> http://static.wp2pcs.com/<?php if($wp2pcs_site_id) echo $wp2pcs_site_id; else echo '~站点号~'; ?>/img/test.jpg （需付费）</label></p>
